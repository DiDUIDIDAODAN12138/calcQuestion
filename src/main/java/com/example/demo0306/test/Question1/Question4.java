...
public class Question4 {
    ...
    public static BigDecimal calc(BigDecimal a, BigDecimal b, BigDecimal c) {
        ...
        BigDecimal total = result1.add(result2).add(result3);
        if (total.compareTo(new BigDecimal(100)) == -1) {
            return total;
        }else {
            BigDecimal multiple = total.divide(new BigDecimal(200)).setScale(0, RoundingMode.DOWN); //满一百减十
            return total.subtract(multiple.multiply(BigDecimal.TEN));
